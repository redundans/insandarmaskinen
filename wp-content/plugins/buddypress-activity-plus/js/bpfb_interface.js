...
var BpfbLinkHandler = function () {
	...
	var createPreviewMarkup = function (data) {
		...
		var html = '<div class="row">' +
					...
					'<div class="bpfb_thumbnail_chooser">' +
						'<i class="icon-chevron-left bpfb_thumbnail_chooser_left"></i>' +
						'&nbsp;' +
						'<i class="icon-chevron-right bpfb_thumbnail_chooser_right"></i>' +
						'&nbsp;' +
						l10nBpfb.choose_thumbnail + '<br />' +
						'<label class="checkbox"><input type="checkbox" id="bpfb_link_no_thumbnail" />Ingen miniatyrbild</label>' +
					'</div>' +
				'</div>';
		$('.bpfb_preview_container').empty().html(html);
		$('.bpfb_action_container').html(
			'<p><input type="button" class="btn btn-small" id="bpfb_submit" value="Posta uppdatering" /> ' +
			'<a href="javascript: _bpfbActiveHandler.destroy(); $(\'#whats-new-options\').show();">Ta bort</a></p>'
		);
	...
	var createLinkPreview = function ( url ) {
