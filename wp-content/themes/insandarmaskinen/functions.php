...
function new_mail_from_name( $old ) {
	global $bp;
	$user_name = bp_get_profile_field_data( array(
...
function insandarmaskinen_ajax_messages_autocomplete_results() {
	...
	if ( ! empty( $users['users'] ) ) {
		...
		foreach ( $users['users'] as $user ) {
			if ( bp_loggedin_user_id() != $user->id ) {
				$user_ids[] = $user->id;
			}
		}
	...
	if ( ! empty( $user_ids ) ) {
		foreach ( $user_ids as $user_id ) {
			...
			echo '<span id="link-' . $username . '" href="' . bp_core_get_user_domain( $user_id ) . '"></span>' .
				...
				' &nbsp;' . bp_core_get_user_displayname( $user_id ) . ' ( ' . $username . ' )' . "\n";
		}
	}
...
function report_paper() {
	...
	foreach ( $papers as $paper ) {
		if ( ! empty( $paper ) ) :
			$term = get_term_by( 'name', trim( $paper ), 'paper' );
			$terms[] = $term->term_id;
		endif;
	}
	...
	if ( null === $terms[0] ) :
		echo json_encode( array(
			'error' => 1,
			'message' => 'Ledsen, vi kunde inte hitta någon tidning med det namnet. Saknar du denna tidning på Insändarmaskinen™ så meddela en administratör i användarforumet så lägger hen till den.',
		) );
		die( );
	else :
