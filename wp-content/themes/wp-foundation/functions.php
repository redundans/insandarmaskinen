...
require_once('library/custom-post-types.php'); // custom post type example
...
if( $test_url !== false ) { // test if the URL exists  
    ...
    function load_local_jQuery() {  
        error_log('Funkade inte att ladda jquery!');
        wp_deregister_script('jquery'); // initiate the function  
        wp_register_script('jquery', get_bloginfo('template_url').'/javascripts/jquery.min.js', __FILE__, false, '1.7.2', true); // register the local file  
...
function wpufe_send_mail( $post_id ) {
    $post_categories = wp_get_post_categories( $post_id );
    $letter = get_post( $post_id );
    $subject = $letter->post_title;
    $message = $letter->post_content;
    foreach($post_categories as $c){
        $cat = get_category( $c );
        $to = get_term_meta($cat->term_id, 'email', true);
        $headers[] = 'From: Jesper <jesnil@gmail.com>';
        ...
        if( $sent )
            ...
            error_log('Something went wrong when trying to send mail to: '.$to.'!');
    }
}
add_action( 'wpuf_add_post_after_insert', 'wpufe_send_mail' );
